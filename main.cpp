...
struct film
{
     string titre;film create_film(string _titre, int _annee, realisateur _nomrealisateur, int _duree, string _genre, acteur _acteur1, acteur _acteur2, acteur _acteur3)
{
    // init var
    film newfilm;
...
acteur create_acteur(string _nom, string _prenom, int _date_de_naissance)
{
    ...
    newacteur.nationalite = _nationalite;
...
realisateur create_realisateur(string _nom, string _prenom, int _date_de_naissance)
{
    ...
    newrealisateur.nationalite = _nationalite;
...
void afficher_infos_film(film filmaverif)
{
    ...
    cout << filmaverif.annee << endl;
    cout << filmaverif.duree << endl;
    cout << filmaverif.genre << endl;
...
void menu_cinema(vector<film> ListeFilms)
{
    ...
    string choix;
    cout << "Choisissez le numéro du film voulu: ";
    cin >> choix;
    ...
    for (unsigned int i = 0; i < ListeFilms.size(); i++)
    {
        if (ListeFilms[i].titre == choix)
        {
            ...
            cout << "Ce film n'est pas à l'affiche !" << endl;
        }
    }
}
